...
/**
 * @file creates and configures the appsrc and appsink elements with libraries and settings from yaml file.
 *  Appsrc is configured with abiRefDataLoader, and appsink with abiRefDataRender
 */

namespace ds3d {

    constexpr uint32_t
    kDataProcessUserDataMagic = NVDS3D_MAGIC_ID('D', '3', 'U', 'D');
...
DS3D_EXPORT_API ds3d::ErrCode NvDs3D_GstAppSrcSetDataloader(GstAppSrc *src, ds3d::abiRefDataLoader *refLoader);
...
namespace gst {
    ...
    template<class GuardProcess>
    inline ErrCode loadCustomProcessor(
            const config::ComponentConfig &compConfig, GuardProcess &customProcessor, Ptr <CustomLibFactory> &lib) {
...
/**
* @brief Generate DataLoaderSrc from config file to override the default appsrc gstreamer element
*
...
*/
    ...
    NvDs3D_CreateDataLoaderSrc(
            const config::ComponentConfig &compConfig,
        ...
        g_object_set(
                G_OBJECT(loaderEle.get()),
                "do-timestamp", TRUE,
                "stream-type", GST_APP_STREAM_TYPE_STREAM,
                "max-bytes", (uint64_t)(kPoolSize * sizeof(NvDs3DBuffer)),
                "min-percent", 80,
                "caps", srcCaps.get(), NULL);

        if (start) {
            DS3D_ERROR_RETURN(
                    loader.start(compConfig.rawContent, compConfig.filePath),
                    "Dataloader start config: %s failed",
                    compConfig.filePath.c_str()
            );
        }

        // ensure that the gstreamer element exists
    ...
    inline ErrCode NvDs3D_CreateDataRenderSink(
            const config::ComponentConfig &compConfig,
        ...
        // set deault element properties on the appsink gstreamer element
        ...
        // set element properties from yaml config file on appsink gstreamer element
        ...
        DS3D_ERROR_RETURN(config::CatchYamlCall(setGstProperties), "parse gst_properties failed for datareander");

        if (start) {
            DS3D_ERROR_RETURN(
                    render.start(compConfig.rawContent, compConfig.filePath),
        ...
        DS_ASSERT(appSink);
        DS3D_ERROR_RETURN(
                NvDs3D_GstAppSinkSetDataRender(appSink, render.release()), "Set datarender into GstAppSink failed."
        );
