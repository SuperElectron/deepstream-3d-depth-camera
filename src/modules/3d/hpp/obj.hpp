...
/**
 * @file core logic to cast GST_BUFFER for the element (appsrc, nvds3dfilter, appsink) using its Guard for safe access
 *
 */

namespace ds3d {

template <class T> using ShrdPtr = std::shared_ptr<T>;
template <class T> using UniqPtr = std::unique_ptr<T, std::function<void(T*)>>;
template <class T> using Ptr = ShrdPtr<T>;

template <class T> void DeleteTFunc(T* t)
{
    if (t) {
        delete t;
    }
...
template <typename T> using _EnableIfValidIdType =
    typename std::enable_if_t<TpId<std::remove_cv_t<T>>::__typeid() >= 0, bool>;

template <typename T, typename... Args> using _IsConstructible = typename std::is_constructible<T, Args...>::value;

template <typename T, typename... Args> using _EnableIfConstructible =
    typename std::enable_if<std::is_constructible<T, Args...>::value, bool>::type;

template <class From, class To> using _Convertible = std::enable_if_t<std::is_convertible<
        From, To>::value ||
        std::is_constructible<std::remove_reference<To>, std::remove_reference<From>>::value,
        bool
>;

template <class Base, class Derived> using _EnableIfBaseOf = std::enable_if_t<std::is_base_of<Base, Derived>::value, bool>;

template <class From, class To> using _PtrConvertible = std::enable_if_t<
...
template <class From, class To, class convertType> To* pointerCast(From* a);

template <class From, class To, _Convertible<From*, To*> = true> To* pointerCast(From* a)
{
    return static_cast<To*>(a);
}

template <class From, class To, _EnableIfBaseOf<From, To> = true> To* pointerCast(From* a)
{
    return dynamic_cast<To*>(a);
}

template <class From, class To, std::enable_if_t<std::is_void<From>::value, bool> = true> To* pointerCast(From* a)
{
    return static_cast<To*>(a);
}

template <class From, class To> class abiRefCast : public abiRefT<To>
{
...
template <class Tp> class SharedRefObj : public abiRefT<Tp>
{
    ShrdPtr<Tp> _ptr;
...
template <class Tp> abiRefT<Tp>* NewAbiRef(Tp* rawAbiObj)
{
...
template <class From, class To = From> inline SharedRefObj<To>* PtrToAbiRef(ShrdPtr<From>&& p)
{
    return new SharedRefObj<To>(std::move(p));
}

template <class From, class To = From> inline ShrdPtr<To> AbiRefToPtr(const abiRefT<From>& p)
{
    if (!p.data())
        return nullptr;
 ...
 */
template <typename... Args> class CBObjT : public abiCallBackT<Args...>
{
    public:
 ...
 *
 */
template <typename ref, _EnableIfBaseOf<abiRefObj, ref> = true> class GuardRef
{
    ref* _abiRef = nullptr;
...
template <typename abiCB> class GuardCB : public GuardRef<abiCB>
{
 ...
 * everywhere
 */
template <class Tp> class GuardDataT : public GuardRef<abiRefT<Tp>>
{
    public:
