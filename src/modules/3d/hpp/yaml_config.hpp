...
/**
 * @file parse the configs from the yaml file for the application
 */
...
template <class F, typename... Args> ErrCode CatchConfigCall(F f, Args&&... args)
{
    ErrCode code = ErrCode::kGood;
...
inline ErrCode parseComponentConfig(const std::string& yamlComp, const std::string& path, ComponentConfig& config)
{
...
inline ErrCode parseFullConfig(const std::string& yamlDoc, const std::string& path, std::vector<ComponentConfig>& all)
{
